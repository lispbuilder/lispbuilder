...
(defmethod _draw-utf8-solid-*_ ((string string) (x integer) (y integer)
                                justify (surface sdl-surface)
                                (font ttf-font) (color color))
  (with-surface (font-surface (_render-utf8-solid_ string font color nil nil) t)
    (set-surface-* font-surface :x x :y y)
    (blit-surface font-surface surface))
  surface)

(defmethod _render-utf8-solid_ ((string string) (font ttf-font) (color color) free cache)
  (let ((surf nil))
    (with-foreign-color-copy (col-struct color)
