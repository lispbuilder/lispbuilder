...
(defpackage :win
  ...
  (:documentation "Windows CFFI mappings"))
...
(defun on-create (hWnd wParam lParam)
  ...
  (with-foreign-object (rect 'RECT)
    ...
    (let* ((width (foreign-slot-value rect 'RECT 'right))
           ...
           (dx (- *width* width))
           (dy (- *height* height))
           (windows-newline (concatenate 'string
                                         (string (code-char 13))
                                         (string (code-char 10)))))
      (MoveWindow hWnd
      ...
      (setf *edit-wnd* (CreateWindowEx
                        0 "EDIT"
                        (concatenate 'string
                                     "(install-timer 100 #'repaint)"
                                     windows-newline "(clear)"
                                     windows-newline)
                        (logior WS_VISIBLE WS_CHILD WS_BORDER ES_LEFT ES_MULTILINE ES_WANTRETURN WS_VSCROLL)
      ...
      (setf *button-wnd* (CreateWindowEx 0 "BUTTON" "Eval"
                                         ...
                                         (- *width* *button-width*) *client-height* *button-width* *text-height*
                                         hWnd (make-pointer +button-id+) (GetModuleHandle (null-pointer)) (null-pointer))))))
...
;      (MessageBox (null-pointer) "button" (format nil "~a" text) 0))))

(defcallback window-proc :long ((hWnd :pointer) (message :unsigned-int) (wParam :unsigned-int) (lParam :long))
  (cond
...
(defun timer ()
  ...
  (let ((start (get-internal-real-time)))
    (install-timer 100 #'(lambda ()
                         (let ((elapsed (- (get-internal-real-time) start)))
                           (draw-string 100 100 (format nil "~a" (floor elapsed INTERNAL-TIME-UNITS-PER-SECOND)))
                           (repaint))))))
