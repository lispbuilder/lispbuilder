...
(defmacro with-c3d ((var &optional (color nil) (free-p nil)) &body body)
  (let ((body-value (gensym "body-value")))
	...
	`(let ((,body-value nil)
	       (,var ,color))
	   (symbol-macrolet ((r (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-3d 'r))
			     (g (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-3d 'g))
			     (b (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-3d 'b)))
...
(defmacro with-c4d ((var &optional color free-p) &body body)
  (let ((body-value (gensym "body-value")))
	...
	`(let ((,body-value nil)
	       (,var ,color))
	   (symbol-macrolet ((r (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-4d 'r))
			     (g (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-4d 'g))
			     (b (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-4d 'b))
			     (a (cffi:foreign-slot-value ,var 'rm-cffi::rm-color-4d 'a)))
	     (setf ,body-value (progn ,@body))
	     (when ,free-p
	       (cffi:foreign-free ,var)))
	   ,body-value)
...
;; (defmacro with-rm-light ((light) &body body) 
;;   (let ((body-value (gensym "body-value")))
;;     `(let ((,body-value nil))
;;        (setf ,body-value (progn ,@body))
...
(defmacro with-v2d ((var &optional (vertex nil) (free-p nil)) &body body)
  (let ((body-value (gensym "body-value")))
    (if vertex
      `(let ((,body-value nil)
             (,var ,vertex))
         (symbol-macrolet ((x (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-2d 'x))
                           (y (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-2d 'y)))
           (setf ,body-value (progn ,@body))
           (when ,free-p
             (cffi:foreign-free ,var)))
         ,body-value)
...
(defmacro with-v3d ((var &optional (vertex nil) (free-p nil)) &body body)
  (let ((body-value (gensym "body-value")))
    (if vertex
      `(let ((,body-value nil)
             (,var ,vertex))
         (symbol-macrolet ((x (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-3d 'x))
                           (y (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-3d 'y))
                           (z (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-3d 'z)))
           (setf ,body-value (progn ,@body))
           (when ,free-p
             (cffi:foreign-free ,var)))
         ,body-value)
...
(defmacro with-v4d ((var &optional (vertex nil) (free-p nil)) &body body)
  (let ((body-value (gensym "body-value")))
	...
	`(let ((,body-value nil)
	       (,var ,vertex))
	   (symbol-macrolet ((x (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-4d 'x))
			     (y (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-4d 'y))
			     (z (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-4d 'z))
			     (w (cffi:foreign-slot-value ,var 'rm-cffi::rm-vertex-4d 'w)))
	     (setf ,body-value (progn ,@body))
	     (when ,free-p
	       (cffi:foreign-free ,var)))
	   ,body-value)
...
(defun v2d+ (addto v2d)
  (incf (cffi:foreign-slot-value addto 'rm-cffi::rm-vertex-3d 'x) (cffi:foreign-slot-value v2d 'rm-cffi::rm-vertex-3d 'x))
  (incf (cffi:foreign-slot-value addto 'rm-cffi::rm-vertex-3d 'y) (cffi:foreign-slot-value v2d 'rm-cffi::rm-vertex-3d 'y))
  addto)

(defun v3d+ (addto v3d)
  (incf (cffi:foreign-slot-value addto 'rm-cffi::rm-vertex-3d 'x) (cffi:foreign-slot-value v3d 'rm-cffi::rm-vertex-3d 'x))
  (incf (cffi:foreign-slot-value addto 'rm-cffi::rm-vertex-3d 'y) (cffi:foreign-slot-value v3d 'rm-cffi::rm-vertex-3d 'y))
  (incf (cffi:foreign-slot-value addto 'rm-cffi::rm-vertex-3d 'z) (cffi:foreign-slot-value v3d 'rm-cffi::rm-vertex-3d 'z))
  addto)
