...
(defun generate-write-pixel (&optional (surface *default-surface*))
  (let* ((format (pixelformat surface))
	 (bpp (foreign-slot-value format 'SDL_PixelFormat 'BytesPerPixel))
	 (pixel-address (foreign-slot-value surface 'SDL_Surface 'Pixels))
	 (pitch (foreign-slot-value surface 'SDL_Surface 'Pitch)))
    (labels ((offset (x y)
	       (+ (* y pitch)
		  (* x bpp)))
      ...
      (let ((fn-write-pixel (generate-write-pixel-fn bpp)))
	#'(lambda (x y color)
	    (funcall fn-write-pixel x y color))))))

(defun generate-read-pixel (&optional (surface *default-surface*))
  (let* ((format (pixelformat surface))
	 (bpp (foreign-slot-value format 'SDL_PixelFormat 'BytesPerPixel))
	 (pixel-address (foreign-slot-value surface 'SDL_Surface 'Pixels))
	 (pitch (foreign-slot-value surface 'SDL_Surface 'Pitch)))
...
(defun draw-pixel (x y &key (surface *default-surface*) (color 0))
