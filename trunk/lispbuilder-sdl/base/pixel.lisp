...
(defun write-pixel (x y color)
  ...
  (error "WRITE-PIXEL must be called within WITH-PIXELS."))

(defun read-pixel (x y color)
  (declare (ignore x y color))
  (error "READ-PIXEL must be called with READ-PIXELS."))
...
(defun generate-read-pixel (surface)
  (let* ((format (pixel-format surface))
	...
	(cffi:with-foreign-objects ((r :unsigned-char) (g :unsigned-char) (b :unsigned-char)
				    (a :unsigned-char))
	  #'(lambda (x y)
	      (sdl-cffi::SDL-Get-RGBA (funcall fn-read-pixel x y) format r g b a)
