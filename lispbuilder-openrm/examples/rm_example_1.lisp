...
(defun rm-example-1 ()
  (let ((width 320) (height 200))
    (sdl::with-init ()
      ...
      (rm::with-init ()
	(rm::with-rmpipe ((get-hwnd) width height) a-pipe
	  (let ((root-node (rm::rmrootnode))
		(a-node (rm::new-node :opacity :opaque)))
	    (rm::node-add-primitive a-node 
				    (rm::new-sphere-primitive :color (rm::color 0.5 0.0 0.1)
							      :position (rm::vertex 0.0 0.0 0.0)
							      :radius 1.0
							      :tesselate 8)
				    (rm::new-sphere-primitive :color (rm::color 0.5 0.0 0.1)
							      :position (rm::vertex 2.0 0.0 0.0)
							      :radius 1.0
							      :tesselate 32)
				    (rm::new-sphere-primitive :color (rm::color 0.5 0.0 0.1)
							      :position (rm::vertex 4.0 0.0 0.0)
							      :radius 1.0
							      :tesselate 128)
				    (rm::new-sphere-primitive :color (rm::color 0.5 0.0 0.1)
							      :position (rm::vertex 6.0 0.0 0.0)
							      :radius 1.0
							      :tesselate 512))
	    (rm::node-add-primitive a-node
				    ; Create a cube by specifying the vertices
				    (rm::new-cube-primitive :color (rm::color 0.5 0.0 0.1)
							    :vertices  (list (rm::vertex -1.0 1.0 -1.0)
									     (rm::vertex 1.0 3.0 1.0)))
				    ; Create a cube by specifying width, height and depth
				    (rm::new-cube-primitive :color (rm::color 0.5 0.0 0.1)
							    :dimensions (rm::vertex 2.0 2.0 2.0)
							    :position (rm::vertex -1.0 -3.0 -1.0)))

;; 	    (rm::node-add-primitive a-node
;; 				    (rm::new-plane-primitive :color (rm::color 0.3 0.9 0.5)
;; 							     :subdivisions 100
;; 							     :vertices (list (rm::vertex -5.0 -8.0 -5.0)
;; 									     (rm::vertex 5.0 -8.0 5.0))))

	    ;Add a bounding box for the node.
;; 	    (rm::node-add-primitive a-node
;; 				    (rm::create-bounds-from-node a-node))
	    ;;Add a bounding box for each primitive in the node.
;; 	    (rm::node-add-primitive a-node
;; 				    (rm::create-bounds-from-node a-node :primitives t))
	    
	    (rm::add-node root-node
			  a-node :union t :compute-center t)

	    (rm::set-node-position a-node (rm::vertex 0.0 0.0 0.0))
...
(defun rm-sin-wave ()
  (let ((width 320) (height 200)
    ...
    (sdl::with-init ()
      (sdl::set-window width height :flags sdl::sdl_opengl)
      (rm::with-engine ((get-hwnd) width height) root-node
