...
(defmethod _render-utf8-shaded_ :around ((string string) (fg-color color) (bg-color color) (font font) free cache)
  (declare (ignore cache))
  (when free
    (free-cached-surface font))
  (call-next-method))

(defmethod _render-utf8-shaded_ ((string string) (fg-color color) (bg-color color) (font bitmap-font) free cache)
  (declare (ignore free))
  (let ((surf (convert-surface :surface (create-surface (* (char-width font)
							   (length string))
							(char-height font))
			       :free t)))
    (_draw-utf8-shaded-*_ string 0 0 fg-color bg-color :LEFT surf font)
    (when cache
      (setf (cached-surface font) surf))
    surf))

(defmethod _draw-utf8-shaded-*_ ((string string) (x integer) (y integer) (fg-color color) (bg-color color) justify (surface sdl-surface) (font bitmap-font))
  (case justify
    (:left (draw-string-left-justify-* string x y fg-color bg-color :surface surface :font font))
    (:right (draw-string-right-justify-* string x y fg-color bg-color :surface surface :font font))
    (:center (draw-string-centered-* string x y fg-color bg-color :surface surface :font font))
...
(defun render-utf8-shaded (string fg-color bg-color &key
			    (font *default-font*)
			    (free nil)
			    (cache nil))
  "Render the string `STRING` using font `FONT` with text color `FG-COLOR` and background color `BG-COLOR`
...
* Also supported in _LISPBUILDER-SDL-GFX_"
  (_render-utf8-shaded_ string fg-color bg-color font free cache))

(defun draw-utf8-shaded (string p1 fg-color bg-color &key
			  (justify :left)
			  (surface *default-surface*)
			  (font *default-font*))
  "See [DRAW-STRING-SHADED-*](#draw-string-shaded-*).
...
* Also supported in _LISPBUILDER-SDL-GFX_"
  (_draw-utf8-shaded-*_ string (x p1) (y p1) fg-color bg-color justify (if surface surface *default-display*) font))

(defun draw-utf8-shaded-* (string x y fg-color bg-color &key
			    (justify :left)
			    (surface *default-surface*)
			    (font *default-font*))
  "Draw text `STRING` at location `X` `Y` using font `FONT` with text color `FG-COLOR` and background color `BG-COLOR`
...
* Also supported in _LISPBUILDER-SDL-GFX_"
  (_draw-utf8-shaded-*_ string x y fg-color bg-color justify (if surface surface *default-display*) font))
