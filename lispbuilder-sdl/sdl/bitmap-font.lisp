...
(defclass bitmap-font (sdl::sdl-font)
...
as well as the most recent surface SDL:SURFACE created by a call to any of the RENDER-STRING* functions. 
...
(defun get-character (char fg-color bg-color &key
  ...
  (let ((redraw? nil)
    ...
    (when redraw?
      (let ((fg-col (sdl:map-color (glyph-fg-color glyph) (glyph-surface glyph)))
	    (bg-col (if bg-color
			(sdl:map-color (glyph-bg-color glyph) (glyph-surface glyph))
			0)))
	(sdl-base::with-pixel (pix (sdl:fp (glyph-surface glyph)))
