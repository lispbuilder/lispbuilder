...
(defmacro with-pixels ((var &optional (surface *default-surface*)) &body body)
  (let ((format (gensym "format-")) (bpp (gensym "bpp-"))
	(pixel-address (gensym "pixel-address-"))
	(pitch (gensym "pitch-")) (fn-write-pixel (gensym "fn-write-pixel-")))
    `(let* (,@(unless var `(,var ,surface))
	    (,format (foreign-slot-value ,var 'SDL_Surface 'format))
	    (,bpp (foreign-slot-value ,format 'SDL_PixelFormat 'BytesPerPixel))
	    (,pixel-address (foreign-slot-value ,var 'SDL_Surface 'Pixels))
	    (,pitch (foreign-slot-value ,var 'SDL_Surface 'Pitch)))
       (labels ((offset (x y)
		  (+ (* y ,pitch)
		     (* x ,bpp)))
		(make-write-pixel (bpp)
		  (case bpp
		    (1 #'(lambda (x y color)
	 ...
	 (let ((,fn-write-pixel (make-write-pixel ,bpp)))
	   (labels ((write-pixel (x y color)
		      (funcall ,fn-write-pixel x y color))
		    (write-point (&key (point *default-position*) (color *default-color*))
		      (write-pixel (sdl:point-x point) (sdl:point-y point) (map-color :color color :surface ,var))))
