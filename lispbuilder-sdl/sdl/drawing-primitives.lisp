        ...
        (flet ((out-code (x y)
                 (let ((code inside))
                   ...
                   (cond ((< y 0) (setq code (logior top code)))
                         ((< ymax x) (setq code (logior bottom code))))
                   code)))
            ...
            (loop
               ...
               (let* (x y
                 ...
                 (cond ((\= 0 (logand oco bottom))
                        (setq x (+ x0 (* (- x1 x0) (/ (- ymax y0) (- y1 y0))))
                              y ymax))
                       ((\= 0 (logand oco top))
                        (setq x (+ x0 (* (- x1 x0) (/ (- y0) (- y1 y0))))
                              y 0))
                       ((\= 0 (logand oco right))
                        (setq y (+ y0 (* (- y1 y0) (/ (- xmax x0) (- x1 x0))))
                              x xmax))
                       ((\= 0 (logand oco left))
                        (setq y (+ y0 (* (- y1 y0) (/ (- x0) (- x1 x0))))
                              x 0)))
                 (if first
                     (setq x0 x y0 y oc0 oco)
                     (setq x1 x y1 y oc1 oco))))
