...
(defmethod _render-utf8-solid_ ((string string) (font bitmap-font) (color color) free cache)
  ...
  (let ((surf (convert-surface :surface (create-surface (* (char-width font)
							...
							:type :hw)
			       :free t)))
    (_draw-utf8-solid-*_ string 0 0 :LEFT surf font color)
    (when cache
      (setf (cached-surface font) surf))
    surf))
...
(defun render-utf8-solid (string &key
                                   (font *default-font*)
                                   (color *default-color*)
                                   (free nil)
                                   (cache nil))
...
(defun draw-utf8-solid (string p1 &key
                                 (justify :left)
                                 (surface *default-surface*)
                                 (font *default-font*)
                                 (color *default-color*))
...
(defun draw-utf8-solid-* (string x y &key
                                   (justify :left)
                                   (surface *default-surface*)
                                   (font *default-font*)
                                   (color *default-color*))
